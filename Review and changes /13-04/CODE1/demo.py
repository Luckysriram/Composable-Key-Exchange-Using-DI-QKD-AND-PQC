...
def demo_basic_pqc():
    ...
    try:
        ...
        if encap_result.success:
            logger.info(f"[OK] Encapsulation successful")
        ...
        if decap_result.success:
            logger.info(f"[OK] Decapsulation successful")
            
            # Verify shared secrets match
            if encap_result.shared_secret == decap_result.shared_secret:
                logger.info("[OK] Shared secrets match!")
                return True
            else:
                logger.error("[FAIL] Shared secrets don't match!")
                return False
        else:
            logger.error(f"[FAIL] Decapsulation failed: {decap_result.error}")
            return False
...
def demo_cukem_hybrid():
    ...
    try:
        ...
        if result.success:
            logger.info(f"[OK] Key exchange successful!")
            ...
            logger.error(f"[FAIL] Key exchange failed: {result.error}")
            return False
    ...
    try:
        ...
        for attempt in range(1, 6):
            try:
                ...
                logger.warning(f"  Attempt {attempt}: [X] {str(e)}")
